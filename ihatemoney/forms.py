...
class EditProjectForm(FlaskForm):
    ...
    logging_preference = IntegerField(_("Logging Preference"), validators=[DataRequired()])
    ...
    def update(self, project):
        """Update the project with the information from the form, only if fields are present."""
        logging.debug(f"Request Form: {request.form}")
        logging.debug(f"Initial Project State: {project.logging_preference}, {project.default_currency}")

        project.name = self.name.data  # Assuming name is always required and included in PUT.

        if 'password' in request.form and self.password.data:
        ...
        # Make sure these checks are included
        # Explicitly handle logging_preference
        if 'logging_preference' in request.form:
            project.logging_preference = self.logging_preference
        else:
            # Maintain the existing value if not provided
            project.logging_preference = project.logging_preference

        if 'default_currency' in request.form:
            project.switch_currency(self.default_currency.data)

        logging.debug(f"Updated Project State: {project.logging_preference}, {project.default_currency}")
