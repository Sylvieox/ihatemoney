...
class ProjectHandler(Resource):
    ...
    def put(self, project):
        form = EditProjectForm(formdata=request.form, obj=project)
        if form.validate():
            project.name = form.name.data
            project.contact_email = form.contact_email.data

            # Update password if provided
            if form.password.data:
                project.password = generate_password_hash(form.password.data)

            # Directly update logging preference
            project.logging_preference = form.logging_preference.data

            # Update currency if it's changed
            if 'default_currency' in request.form:
                project.default_currency = form.default_currency.data
